...
poco is a relatively bare-bones library, as such, it requires little from the underlying
...
poco includes platform information via `#include <poco/platform.h>`, ensure
...
- `platform_swap_context()` performs the get operation, followed by a set operation.
- `platform_make_context()` modifies the input context to jump to the defined
  entrypoint.
...
3. Tick arithmetic is always performed as differences between 2 points. The value of the
   tick at any given point is not relevant.

- `PlatformTick` type definition of the tick unit.
...
of interrupts, it may be required to mask interrupts for a short duration to ensure
state updates remain "atomic" relative to the scheduler.
