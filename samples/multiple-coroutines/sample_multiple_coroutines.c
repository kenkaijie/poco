...
// As we are creating a large number of homogenous coroutines, we just do it as an array.

static coro_t coroutines[CORO_COUNT];
static uint32_t coroutine_stacks[CORO_COUNT][STACK_SIZE] __attribute__((__aligned__(8)));
