...
static error_t _notify_from_isr(round_robin_scheduler_t * scheduler, coro_event_source_t const * event)
{
    return queue_raw_put(&scheduler->event_queue, (void *)event);
}

static error_t _notify(round_robin_scheduler_t * scheduler, coro_event_source_t const * event)
{
 ...
 */
static void _update_waiting_tasks(round_robin_scheduler_t * scheduler, coro_event_source_t const * event)
{
...
round_robin_scheduler_t * round_robin_scheduler_create_static(round_robin_scheduler_t * scheduler, coro_t * const* coro_list, size_t num_coros)
{
    ...
    scheduler->scheduler.notify_from_isr = (scheduler_notify_from_isr_t)_notify_from_isr;
    ...
    queue_create_static(&scheduler->event_queue, SCHEDULER_MAX_EXTERNAL_EVENT_COUNT, sizeof(coro_event_source_t), (uint8_t *)scheduler->external_events);
