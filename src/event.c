...
static bool _event_triggered(event_t * event, flags_t mask, bool wait_for_all)
{
    ...
    return (wait_for_all) ? ((event->flags & mask) == mask) : ((event->flags & mask) != 0);
...
flags_t event_get(coro_t * coro, event_t * event, flags_t mask, flags_t clear_mask, bool wait_for_all)
{
