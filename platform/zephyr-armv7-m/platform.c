...
__attribute__((optimize("omit-frame-pointer"))) static void trampoline(void) {
    ...
    platform_set_context(uc_link); // Restore the context of the link register
}
 ...
 */
int platform_get_context(platform_context_t *ucp) {
    __asm__ volatile(
        "add r1, %0, #0x1C\n"   // offset to the machine context, skipping first 4
                                // registers (12 + 16)
...
int platform_swap_context(platform_context_t *ocp, const platform_context_t *ucp) {
    ...
    __asm__ volatile(
        ...
        // FIXME: we need to restore the PC to the value of R15 in the context, but we
        // cannot do this as we need to restore all registers first, so that information
        // is lost?
        "mov pc, r3\n"       // Branch to the link register (return)
        :                    // No output operands
